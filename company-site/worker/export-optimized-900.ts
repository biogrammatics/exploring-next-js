/**
 * Export 900 more DP-optimized DNA sequences (opt101-opt1000)
 * from the human proteome, skipping the first 100 already exported.
 *
 * Run: npx tsx worker/export-optimized-900.ts
 */
...
async function main() {
  ...
  const SKIP_COUNT = 100; // skip the first 100 candidates (already exported)
  const START_NUM = 101;  // numbering starts at opt101
  const MIN_LEN = 400;
  const MAX_LEN = 700;
  ...
  console.log(`Skipping first ${SKIP_COUNT} candidates (already exported as opt001-opt100)`);

  const remainingCandidates = candidates.slice(SKIP_COUNT);
  console.log(`Remaining candidates: ${remainingCandidates.length}`);

  if (remainingCandidates.length < TARGET_COUNT) {
    console.error(
      `Not enough candidate proteins (${remainingCandidates.length} < ${TARGET_COUNT})`
    );
    process.exit(1);
  }
  ...
  console.log(
    `DP OPTIMIZER -- Targeting ${TARGET_COUNT} proteins (opt${START_NUM}-opt${START_NUM + TARGET_COUNT - 1})`
  );
  console.log("=".repeat(80));
  ...
  for (const candidate of remainingCandidates) {
    ...
    if (result.success && result.dnaSequence) {
      ...
      if (results.length % 100 === 0 || results.length === 1) {
        const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
        const rate = results.length > 1 ? (results.length / ((Date.now() - startTime) / 1000)).toFixed(1) : "—";
        console.log(
          `  [${results.length.toString().padStart(3)}/${TARGET_COUNT}] ` +
            `${elapsed}s elapsed, ${rate} prot/s, ${skipped} skipped`
        );
      }
    } else {
      skipped++;
  ...
  const outputPath =
    "/Users/tom/Claude/codon-optimization/dp_optimized_900.fasta";
  const fastaLines: string[] = [];
  const mappingLines: string[] = ["#\tUniProt_ID\tAA_len\tDNA_len\tScore"];

  for (let i = 0; i < results.length; i++) {
    const r = results[i];
    const num = (START_NUM + i).toString().padStart(4, "0");
  ...
  // Write mapping file
  const mappingPath =
    "/Users/tom/Claude/codon-optimization/dp_optimized_900_mapping.tsv";
  ...
  console.log("SUMMARY");
  console.log("=".repeat(80));
  console.log(`Candidates available: ${remainingCandidates.length}`);
  ...
  console.log(`  ${results.length} sequences (named opt${START_NUM.toString().padStart(4, "0")}-opt${(START_NUM + results.length - 1).toString().padStart(4, "0")}, each with TAA stop)`);
