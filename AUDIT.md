...
| 🟡 **M56** | Arch. | Worker randomly resolves B/Z/J/X residues and substitutes U/O; `targetOrganism` is accepted but never selects anything. | 🟡 Partial — 2026-09-13: intake and worker now accept only the 20 standard amino acids (plus one trailing `*`); ambiguity codes and U/O are rejected with a per-character explanation shown to the customer, never substituted. Still open: store algorithm version and parameters with the job; reject unknown `targetOrganism` values. Raised from L25. |
